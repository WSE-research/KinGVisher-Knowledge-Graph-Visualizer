...
RUN (export DRY_RUN=True; streamlit run knowledge_graph_visualizer.py &) && sleep 5 && curl http://localhost:${SERVICE_PORT}/

HEALTHCHECK CMD curl --fail http://localhost:${SERVICE_PORT}/_stcore/health

ENTRYPOINT streamlit run knowledge_graph_visualizer.py --server.port=${SERVICE_PORT} --server.address=0.0.0.0